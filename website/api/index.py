...
# -----------------------------
# YARA Rule Loading (Folder Mode) - FULLY RESTORED
# -----------------------------
...
def scan():
    ...
    try:
        ...
        if "file" in request.files:
            ...
            if not match_details:
                ...
                if fuzzy_hits:
                    ...
                    analysis_log.append(f"🔍 YARA Missed. Fuzzy DB found {len(fuzzy_hits)} matches.")
        ...
        # -----------------------------
            ...
            def process_asset(asset_type, asset_path):
                try:
                    ...
                    r = requests.get(full_url, timeout=2, headers={"User-Agent": "Mozilla/5.0"}, verify=False)
                    
                    if r.status_code != 200 or len(r.content) > 500 * 1024: return None
                    text = r.content.decode("utf-8", errors="ignore")
                    matches_local = rules.match(data=r.content)
                    return {"type": asset_type, "name": asset_path.split("/")[-1], "content": text[:2000], "matches": [format_match(m, build_snippets(text, m)) for m in matches_local], "bytes": r.content}
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    if data:
                        ...
                        content_preview[data["type"]].append({"name": data["name"], "content": data["content"]})
            ...
            if not match_details:
                ...
                if fuzzy_hits:
                    match_details.extend(fuzzy_hits)
                    analysis_log.append("🔍 YARA Missed. Fuzzy DB found structural variants.")

        else: return jsonify({"error": "No content provided"}), 400

        # -----------------------------
        # HEURISTIC ANALYSIS (RESTORED)
        # -----------------------------
        heuristics_score = 0
        try:
        ...
        if len(redirect_matches) >= 3:
            ...
            heuristics_score += 20
            snips = [decoded_content[max(0, rm.start()-60):min(len(decoded_content), rm.end()+60)].strip() for rm in redirect_matches[:3]]
            heuristic_details.append({"rule": "Suspicious Redirects", "snippets": snips})
        else:
        ...
        if obf_count >= 8:
            ...
            snips = [decoded_content[max(0, om.start()-60):min(len(decoded_content), om.end()+60)].strip() for om in list(obf_re.finditer(decoded_content))[:3]]
            heuristic_details.append({"rule": "Eval/Obfuscation", "snippets": snips})
        else:
