...
import { getThreatInfo } from "@/data/threatDescriptions";

type ContentAsset = { name: string; content: string };
type MatchDetail = { rule: string; snippets: string[]; yaraRule?: string };

type ScanResult = {
    score: number;
    matches: MatchDetail[];
    analysis_log: string[];
    content_preview: string | { html: string; js: ContentAsset[]; css: ContentAsset[] };
    details?: string;
};
...
export default function Scanner() {
    ...
    const [scrapedTab, setScrapedTab] = useState<"html" | "js" | "css">("html");
                                    ...
                                    {result.matches.length > 0 ? (
                                        <div className="space-y-2">
                                            {result.matches.map((match, idx) => {
                                                const matchRule = typeof match === "string" ? match : match.rule;
                                                const matchSnippets = typeof match === "string" ? [] : (match.snippets || []);
                                                const info = getThreatInfo(matchRule);
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <button
                                                            ...
                                                            className="w-full p-3.5 text-left transition-colors duration-200 hover:bg-white/[0.03]"
                                                        >
                                                            <div className="flex items-start gap-3">
                                                                ...
                                                                <div className="min-w-0 flex-1">
                                                                    <div className="flex items-center justify-between">
                                                                        ...
                                                                        <ChevronDown
                                                                            className="w-3.5 h-3.5 transition-transform duration-300 shrink-0 ml-2"
                                                                            style={{
                                                                                color: "var(--text-tertiary)",
                                                                    ...
                                                                    >
                                                                        ...
                                                                        {matchSnippets.length > 0 && (
                                                                            <div>
                                                                                ...
                                                                                >
                                                                                    Captured Evidence ({matchSnippets.length})
                                                                                </div>
                                                                                <div className="space-y-1.5">
                                                                                    {matchSnippets.map((snippet, si) => (
                                                                                        <pre
                                                                                            key={si}
                                                                                            className="p-2.5 rounded-md text-[11px] leading-relaxed overflow-x-auto"
                                                                                            style={{
                                                                        ...
                                                                        {/* YARA Rule */}
                                                                        {(match.yaraRule || matchRule.includes("Redirect") || matchRule.includes("Obfuscation") || matchRule.includes("Eval")) && (
                                                                            <div>
                                                                                <div
                                                                                    className="text-[10px] uppercase tracking-wider font-semibold mb-1.5"
                                                                                    style={{
                                                                                ...
                                                                                >
                                                                                    {match.yaraRule ? "YARA Rule" : "Detection Rule"}
                                                                                </div>
                                                                                <pre
                                                                                    className="p-2.5 rounded-md text-[11px] leading-relaxed overflow-x-auto"
                                                                                    style={{
                                                                                ...
                                                                                >
                                                                                    {match.yaraRule || `Heuristic Check (not a YARA rule)\nPattern-based behavioral analysis`}
                                                                                </pre>
                                                                            </div>
                                                                        )}
                                                                        ...
                                                                        {info.learnMore && (
                                                                            <a
                                                                                ...
                                                                                className="inline-flex items-center gap-1.5 text-[11px] font-medium transition-colors duration-200 hover:underline"
                                                                                style={{
                                                                                    color: "var(--cyan-dim)",
                                                                                    fontFamily: "var(--font-jetbrains), monospace",
                                                                                }}
                                    ...
                                    {showSource && (
                                        ...
                                        >
                                            <div style={{ borderTop: "1px solid var(--border)" }}>
                                                {/* Tab bar — only show for structured previews */}
                                                {typeof result.content_preview === "object" ? (
                                                    <>
                                                        <div
                                                        ...
                                                        >
                                                            {(["html", "js", "css"] as const).map((tab) => {
                                                                const preview = result.content_preview as { html: string; js: ContentAsset[]; css: ContentAsset[] };
                                                                const count = tab === "html" ? null : preview[tab].length;
                                                                const isActive = scrapedTab === tab;
                                                                return (
                                                                    <button
                                                                        key={tab}
                                                                        onClick={() => setScrapedTab(tab)}
                                                                        className="px-4 py-2.5 text-xs font-medium uppercase tracking-wider transition-all duration-200"
                                                                        style={{
                                                                            fontFamily: "var(--font-jetbrains), monospace",
                                                                    ...
                                                                    >
                                                                        ...
                                                                        {count !== null && (
                                                                            <span
                                                                                ...
                                                                                style={{
                                                                                    background: isActive ? "var(--cyan-glow)" : "rgba(255,255,255,0.05)",
                                                        ...
                                                        {/* Tab content */}
                                                        <pre
                                                            className="p-4 text-xs overflow-x-auto max-h-96 leading-relaxed overflow-y-auto"
                                                            style={{
                                                        ...
                                                        >
                                                            {scrapedTab === "html" && (
                                                                (result.content_preview as { html: string }).html || "No HTML content."
                                                            )}
                                                            {scrapedTab === "js" && (() => {
                                                                const assets = (result.content_preview as { js: ContentAsset[] }).js;
                                                                if (!assets.length) return "No JavaScript files found.";
                                                                return assets.map((a, i) =>
                                                                    `${"─".repeat(3)} ${a.name} ${"─".repeat(3)}\n${a.content}`
                                                                ).join("\n\n");
                                                            })()}
                                                            {scrapedTab === "css" && (() => {
                                                                const assets = (result.content_preview as { css: ContentAsset[] }).css;
                                                                if (!assets.length) return "No CSS files found.";
                                                                return assets.map((a, i) =>
                                                                    `${"─".repeat(3)} ${a.name} ${"─".repeat(3)}\n${a.content}`
                                                                ).join("\n\n");
                                                            })()}
                                                    ...
                                                    /* File upload: plain string preview */
                                                    <pre
                                                        className="p-4 text-xs overflow-x-auto max-h-96 leading-relaxed overflow-y-auto"
                                                        style={{
                                                            background: "rgba(0,0,0,0.3)",
                                                    ...
                                                    >
                                                        {(result.content_preview as string) || "No content available."}
                                                    </pre>
                                                )}
                                            </div>
                                        </motion.div>
